...
public class PlayerController : MonoBehaviour
{
    ...
    public float maxVelRotHor = 1f;
    public float maxVelRotVer = 1f;
    private void Awake()
    {
    ...
    {
        if (playerCamera.GetComponent<Telekinesis>().heldObj != null)
        {
            rotationInput.x = Mathf.Clamp(Input.GetAxis("Mouse X"), -maxVelRotHor, maxVelRotHor) * rotationSensitivity;
            rotationInput.y = -Mathf.Clamp(Input.GetAxis("Mouse Y"), -maxVelRotVer, maxVelRotVer) * rotationSensitivity;
        }
        else
        {
